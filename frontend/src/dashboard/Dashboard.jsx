...
import { ArrowUpRight, Leaf, Sparkles, Package, Boxes, ShoppingCart, Users, DollarSign } from "lucide-react";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    getDashboardStats()
      .then(setStats)
      .catch((e) => setStatsError(e.response?.data?.message || "Could not load dashboard data."))
      .finally(() => setStatsLoading(false));
  }, [isHome]);

  const cards = stats
    ? [
        { title: "Total Products", value: stats.products, growth: null, subtitle: "live count", tone: "emerald", icon: Package },
        { title: "Total Categories", value: stats.categories, growth: null, subtitle: "live count", tone: "blue", icon: Boxes },
        { title: "Total Orders", value: stats.orders, growth: null, subtitle: "all time", tone: "amber", icon: ShoppingCart },
        { title: "Total Customers", value: stats.customers, growth: null, subtitle: "registered users", tone: "rose", icon: Users },
        { title: "Revenue", value: stats.revenue, growth: null, subtitle: "excl. cancelled", tone: "emerald", icon: DollarSign },
      ]
    : [];
          ...
          >
            {isHome ? (
              <>
                ...
                {statsError && (
                  <p className="rounded-2xl bg-rose-50 p-4 text-sm text-rose-600">{statsError}</p>
                )}
                ...
                <div className="grid gap-5 xl:grid-cols-[1.45fr_0.95fr]">
                  <SalesChart data={stats?.monthly || []} loading={statsLoading} />
                  <PieAnalytics data={stats?.categoryShare || []} loading={statsLoading} />
                </div>
                <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
                  <RevenueChart data={stats?.monthly || []} loading={statsLoading} />
                  <QuickActions />
                </div>
                <div className="grid gap-5 xl:grid-cols-[1.25fr_0.75fr]">
                  <OrdersTable orders={stats?.recentOrders || []} loading={statsLoading} />
                  <div className="space-y-5">
                    <TopProducts products={stats?.topProducts || []} loading={statsLoading} />
                    <LowStock items={stats?.lowStock || []} loading={statsLoading} />
                  </div>
                </div>
              </>
            ) : (
