...
const normaliseVariations = (variations) => variations.map((variation) => ({
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Add every pouch pack with its own price. Customers can select these options on the product page.
          </p>
        </div>
        <button
          type="button"
          ...
          {variations.map((variation, index) => (
            <div key={index} className="rounded-xl border border-slate-200 bg-white p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <p className="font-semibold text-slate-700">Variation {index + 1}</p>
                <button
                  type="button"
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                <label className="text-sm font-semibold text-slate-700">
                  Pouches
                  <input name="pouches" type="number" min="1" value={variation.pouches} onChange={(event) => onChange(index, event)} required className="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-blue-500" />
                </label>
                <label className="text-sm font-semibold text-slate-700">
                  MRP
                  <input name="mrp" type="number" min="0" value={variation.mrp} onChange={(event) => onChange(index, event)} required className="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-blue-500" />
                </label>
                <label className="text-sm font-semibold text-slate-700">
                  Selling price
                  <input name="price" type="number" min="0" value={variation.price} onChange={(event) => onChange(index, event)} required className="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-blue-500" />
                </label>
                <label className="text-sm font-semibold text-slate-700">
                  Variation stock
                  <input name="stock" type="number" min="0" value={variation.stock} onChange={(event) => onChange(index, event)} className="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-blue-500" />
                </label>
                <label className="text-sm font-semibold text-slate-700">
                  Label (optional)
                  <input name="name" value={variation.name} onChange={(event) => onChange(index, event)} placeholder={`${variation.pouches || "28"} pouches`} className="mt-1.5 w-full rounded-lg border border-slate-200 px-3 py-2 outline-none focus:border-blue-500" />
                </label>
              </div>
              <label className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-slate-700">
                <input name="isActive" type="checkbox" checked={variation.isActive} onChange={(event) => onChange(index, event)} className="h-4 w-4 accent-blue-600" />
  ...
  return (
    <>
      <div className="grid gap-4 md:grid-cols-2">
        <label className="text-sm font-semibold text-slate-700">Product name<input name="name" value={form.name} onChange={onChange} required className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 outline-none focus:border-blue-500" /></label>
        <label className="text-sm font-semibold text-slate-700">Category<select value={form.categoryId} onChange={onCategoryChange} required className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 outline-none focus:border-blue-500"><option value="">Select category</option>{categories.filter((category) => category.isActive || category._id === form.categoryId).map((category) => <option key={category._id} value={category._id}>{category.name}</option>)}</select></label>
        <label className="text-sm font-semibold text-slate-700">Brand<input name="brand" value={form.brand} onChange={onChange} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 outline-none focus:border-blue-500" /></label>
        <label className="text-sm font-semibold text-slate-700">Stock<input name="stock" type="number" min="0" value={form.stock} onChange={onChange} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 outline-none focus:border-blue-500" /></label>
        <label className="text-sm font-semibold text-slate-700">Default MRP<input name="mrp" type="number" min="0" value={form.mrp} onChange={onChange} required className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 outline-none focus:border-blue-500" /></label>
        <label className="text-sm font-semibold text-slate-700">Default selling price<input name="sellingPrice" type="number" min="0" value={form.sellingPrice} onChange={onChange} required className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2.5 outline-none focus:border-blue-500" /></label>
      ...
      <label className="flex items-center gap-2 text-sm font-semibold text-slate-700"><input name="featured" type="checkbox" checked={Boolean(form.featured)} onChange={onChange} className="h-4 w-4 accent-blue-600" /> Featured product</label>
      {actionError && <p className="rounded-lg bg-rose-50 p-3 text-sm text-rose-600">{actionError}</p>}
      <div className="flex justify-end"><button type="submit" disabled={saving} className="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-60">{saving ? "Saving..." : submitLabel}</button></div>
    </>
  );
}
...
export default function ProductsPage() {
  ...
  const updateVariation = (setFormState) => (index, event) => {
    ...
    setFormState((current) => ({
      ...current,
      variations: current.variations.map((variation, variationIndex) => variationIndex === index
        ? { ...variation, [name]: type === "checkbox" ? checked : value }
        : variation),
    }));
  };

  const addVariation = (setFormState) => () => setFormState((current) => ({ ...current, variations: [...current.variations, emptyVariation()] }));
  const removeVariation = (setFormState) => (index) => setFormState((current) => ({ ...current, variations: current.variations.filter((_, variationIndex) => variationIndex !== index) }));
  const selectCategory = (setFormState) => (event) => {
    const selected = categories.find((category) => category._id === event.target.value);
    setFormState((current) => ({ ...current, categoryId: selected?._id || "", category: selected?.name || "" }));
  };
  ...
  const saveProduct = async (event) => {
    ...
    setActionError("");
    try {
      await dispatch(updateproduct({ id: editingProduct._id, data: { ...form, variations: normaliseVariations(form.variations) } })).unwrap();
      await dispatch(fetchCategories());
  ...
  const columns = [
    ...
    { key: "status", label: "Status", render: (item) => <StatusBadge tone={item.status === "Active" ? "green" : "rose"}>{item.status || "Draft"}</StatusBadge> },
    { key: "featured", label: "Featured", render: (item) => item.featured ? <StatusBadge>Yes</StatusBadge> : <span className="text-slate-400">No</span> },
    { key: "actions", label: "Actions", render: (item) => <div className="flex items-center gap-2"><button type="button" onClick={() => startEdit(item)} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-50 px-3 py-2 text-xs font-semibold text-blue-700 transition hover:bg-blue-100" aria-label={`Edit ${item.name}`}><Pencil size={15} /> Edit</button><button type="button" onClick={() => removeProduct(item)} className="inline-flex items-center gap-1.5 rounded-lg bg-rose-50 px-3 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-100" aria-label={`Delete ${item.name}`}><Trash2 size={15} /> Delete</button></div> },
  ];

  return <AdminSectionPage title="All Products" description="Manage inventory, pricing, stock, and product visibility." badge="Catalog" action={<button type="button" onClick={() => { setActionError(""); setAddForm(emptyProductForm()); setAddingProduct(true); }} className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-blue-600/20 transition hover:bg-blue-700"><Plus size={18} /> Add Product</button>}>
