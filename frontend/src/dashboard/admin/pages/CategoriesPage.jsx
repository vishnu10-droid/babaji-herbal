...
export default function CategoriesPage() {
  ...
  const saveCategory = async (event) => {
    ...
    setActionError("");
    try {
      await dispatch(updateCategorydata({ id: editingCategory._id, data })).unwrap();
  ...
  const columns = [
    { key: "image", label: "Image", render: (category) => category.image ? <img src={`${API_ORIGIN}${category.image}`} alt={category.name} className="h-12 w-12 rounded-xl object-cover" /> : <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-100 text-xs text-blue-600">N/A</div> },
